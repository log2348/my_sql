...
-- 문제 2. '그대 내 품에'라는 노래를 부른 아티스트를 검색하라.
SELECT a.아티스트
FROM 앨범 a
INNER JOIN 곡 b
ON a.앨범번호 = b.앨범번호
...
SELECT a.앨범번호, b.타이틀, COUNT(a.앨범번호) AS '수록곡 수'
FROM 곡 a
INNER JOIN 앨범 b
ON a.앨범번호 = b.앨범번호
GROUP BY b.타이틀;
...
-- 문제 7. 아티스트와 타이틀이 동일한 앨범의 타이틀을 검색하라.
SELECT 타이틀
FROM 앨범
WHERE 아티스트 = 타이틀;
